...
void push_swap_algorithm(Node_a **stack_a, Node_b **stack_b) {
          ...
          while (current_b) {
                    if (current_b->data > max) {
                              max = current_b->data;
                    }
                    if (current_b->data < min) {
                              min = current_b->data;
                    }
                    current_b = current_b->next;
          }
          ...
          while (*stack_a) {
                    ...
                    temp = temp->next;
                              ...
                              if (cheapest_node) {
                                        //printf("Top of Stack A: %d\n", (*stack_a)->data); // Affichez le sommet de la pile A
                                        int rotations_a = 0;
                                        ...
                                        // Calcul de la taille de la pile pour déterminer si c'est plus pertinent d'utiliser ra ou rra
                                        int stack_a_size = 0;
                                        current = *stack_a;
                                        while (current != NULL) {
                                                  current = current->next;
                              ...
                              if(ft_lstsize(*stack_a) == 1 && (*stack_a)->data == max) {
                                        break;
                              } else {
                                        pb(stack_a, stack_b);
                              }

                              // Après chaque coup, réévaluez les coûts
                              cheapest = INT_MAX;
                              cheapest_node = NULL;
                    }

                    // Réévaluez les coûts après chaque coup
                    cheapest = INT_MAX;
                    cheapest_node = NULL;
          ...
          while (*stack_b) {
                    int cheapest = INT_MAX;
                    Node_b *cheapest_node = NULL;
                    Node_b *temp = *stack_b; // Utilisez une variable temporaire
                    int operations = 0;
                    ...
                    if (temp->data == max || temp->data == min) {
                              ...
                              // Calculer le nombre de rotations nécessaires pour déplacer l'élément vers le haut de la pile B
                              Node_b *temp2 = *stack_b; // Utilisez une autre variable temporaire pour les rotations
                              while (temp2 && temp2->data != max) {
                                        temp2 = temp2->next;
                                        rotations_a++;
                              }

                              temp2 = *stack_b; // Réinitialisez temp2
                              while (temp2 && temp2->data != min) {
                                        temp2 = temp2->next;
                                        rotations_b++;
                              }
                    ...
                    temp = temp->next;
                              ...
                              if (cheapest_node) {
                                        //printf("Top of Stack B: %d\n", (*stack_b)->data); // Affichez le sommet de la pile A
                                        int rotations_b = 0;

                                        current_b = *stack_b;
                                        while (current_b != NULL && current_b != cheapest_node) {
                                                  current_b = current_b->next;
                                                  rotations_b++;
                                        }
